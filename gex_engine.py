...
ANALYSES_FILE = "ticker_analyses.json"
OPTIONS_FILE = "active_options.json"
CANDIDATES_FILE = "candidate_stocks.json"
SCANS_DIR = "scans"
...
def calculate_grade(ticker, spot, ptrans, ntrans, gex, cotmp, extra_rules=None):
    """
    Grades setup based on 11 rules.
    extra_rules can supply the boolean values for the rules that need deep data.
    """
...
def cmd_analyze(args):
    ...
    if hasattr(args, "inst_file") and args.inst_file and hasattr(args, "quote_file") and args.quote_file:
        try:
            ...
            instruments_list = inst_data.get("instruments", []) if isinstance(inst_data, dict) else inst_data
            if not isinstance(instruments_list, list) and isinstance(inst_data, dict) and "data" in inst_data:
...
def persist_new_scans():
    """
    Scans the current directory for raw Robinhood scan downloads.
    ...
    then deletes the temporary source file from the workspace root.
    """
    cwd = "."
    ...
    # Look for JSON files in the current working directory
    for file in os.listdir(cwd):
        if file.endswith(".json") and file not in (REGIME_FILE, ANALYSES_FILE, OPTIONS_FILE, CANDIDATES_FILE):
            ...
            try:
                ...
                if scan_title and scan_id:
                    ...
                    timestamped_path = os.path.join(SCANS_DIR, timestamped_filename)
                    ...
                    # Delete the temporary source file to keep workspace clean
                    os.remove(filepath)
