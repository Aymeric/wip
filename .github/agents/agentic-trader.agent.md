...
1. **Identify Target Account**: Call `robinhood-mcp_get_accounts`. Locate the account with `agentic_allowed: true`.
2. **Buying Power Sanity**: Call `robinhood-mcp_get_portfolio` for that specific account and check the cash balance/buying power.
...
1. Before drafting any trade, call `robinhood-mcp_get_equity_tradability` with the target symbol and the active account number.
...
2. Isolate the exact contract identification:
   - Call `mcp_robinhood-tra_get_option_chains` with the underlying symbol.
   - Choose the expiration (closest to 30-45 calendar days out, avoiding short weekly decays).
   - Call `mcp_robinhood-tra_get_option_instruments` to locate the target strike and option type (`call` or `put`).
...
#### For Equities (Stocks/ETFs):
1. Call `robinhood-mcp_review_equity_order` to perform a dry-run check.
...
#### For Options:
1. Call `robinhood-mcp_review_option_order` with the selected `chain_symbol`, `legs` listing the `option_id`, `side` (`buy`), and `position_effect` (`open`).
2. Log and review:
...
3. Call `robinhood-mcp_place_equity_order` or `robinhood-mcp_place_option_order`.

---

### Step 5: Sync Trade to Local GEX Engine
Once an option order completes:
1. Capture the final executed premium, strike, and expirations.
2. Register the position in the local GEX Gating CLI tracker by running:
...
3. This adds the position to [data/active_positions.json](../../data/active_positions.json), bringing it under the strict trailing-stop governance checked via `python3 src/gex_engine.py portfolio` stops validation.
...
- **GEX Engine database updated**: Merged position to [data/active_positions.json](../../data/active_positions.json)
```
